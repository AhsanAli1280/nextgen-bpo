...
import { Building2, Briefcase, Rocket, ShoppingCart, CheckCircle, ArrowRight, LucideIcon } from 'lucide-react';
import { Container } from '@/components/ui/container';
import { SectionHeading } from '@/components/ui/section-heading';
import { INDUSTRIES } from '@/lib/data/industries';
import { getGradientClasses } from '@/lib/utils';

const ICON_MAP: Record<string, LucideIcon> = { Building2, Briefcase, Rocket, ShoppingCart };

export function IndustryExpertise() {
  return (
    <section id="industries" className="py-20 lg:py-28 bg-brand-light">
      <Container>
        <SectionHeading title="Built for Your Industry" subtitle="Deep domain expertise in the sectors where precision, compliance, and scalability matter most." />
        <motion.div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6" variants={{ animate: { transition: { staggerChildren: 0.1, delayChildren: 0.2 } } }} initial="initial" whileInView="animate" viewport={{ once: true }}>
          {INDUSTRIES.map((industry, index) => {
            ...
            return (
              <motion.div key={industry.id} className="group bg-white rounded-2xl p-6 shadow-sm hover:shadow-xl transition-all duration-300 border border-brand-border/60 hover:border-brand-blue/30" initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.1 }}>
        ...
        <motion.p className="text-center text-sm text-brand-gray mt-8" initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ delay: 0.3 }}>
          Not seeing your industry?{' '}<a href="#contact" className="text-brand-blue hover:text-brand-dark font-medium">We adapt to your needs →</a>
        </motion.p>
      </Container>
    </section>
  );
